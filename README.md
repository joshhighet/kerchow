...
[:link:](sbin/boing) _make bebop noise_

## cats

[:link:](sbin/cats) _print the source code of any script in this directory_

## certinfo

[:link:](sbin/certinfo) _return a jsonified str of the x509 for a given url_
...
[:link:](sbin/checkmsuser) _check if a given email is associated with m365_

## chrome

[:link:](sbin/chrome) _run a command line chrome that prints full dom after js processing_

## ci-domain

[:link:](sbin/ci-domain) _open a domain in browser sessions for cybertasks_

## ci-ip

[:link:](sbin/ci-ip) _open an ip in browser sessions for cyebrtasks_

## ci-term

[:link:](sbin/ci-term) _search for a string across places_
...
[:link:](sbin/crtsh) _use the crt.sh api to lookup a domain for certificates_

## cruises

[:link:](sbin/cruises) _fetch incoming ships and parse using beautifulsoup_
...
[:link:](sbin/dex) _get a shell in the latest started local container_

## digall

[:link:](sbin/digall) _query google DNS server for all available record types_
...
[:link:](sbin/edns) _get client subnet & dns resolver from ip-api dot com_

## epoch

[:link:](sbin/epoch) _convert epoch int to local datetime_

## erain

[:link:](sbin/erain) _make emoji matrixrain_
...
[:link:](sbin/finfo) _quickly retrieve get file info and hashes_
...
[:link:](sbin/flushdns) _flush dns cache on macOS (monterey)_
...
[:link:](sbin/getmstenant) _get the m365 tenantid for a given domain_
...
[:link:](sbin/getwordlists) _fetch a heap of wordlists for... science_

## gf

[:link:](sbin/gf) _show git tracked changes of a file_
...
[:link:](sbin/gitsubrm) _remove a submodule from a git repo_
...
[:link:](sbin/google) _googles something in default browers, macos only_
...
[:link:](sbin/http-scanner) _run a suite of url checks for the cyber ga gb gc gf gl gp gs kh ns wa_
...
[:link:](sbin/kh) _healthcheck a kubes service or all services_
...
[:link:](sbin/kstart) _scale a kuberenetes service up to one or bring all up if unspecified_
...
[:link:](sbin/mailcheck) _lookup SPF & DMARC records for a domain_
...
[:link:](sbin/projectsonar) _use the dnsgrep api provided by bufferover.run to lookup a domain_
...
[:link:](sbin/showdupes) _show duplicate lines within a file_

## sonarscan

[:link:](sbin/sonarscan) _scan your current dir for sonar cloud_
...
[:link:](sbin/threatcrowd) _use the threatcrowd api to lookup a domain_
...
[:link:](sbin/whatport) _search for common port usages (i.e 80 is http)_

## zonetransfer

[:link:](sbin/zonetransfer) _attempt an DNS AXFR (zone transfer) with dig_
